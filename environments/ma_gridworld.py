...
class MAGridworld(object):
    ...
    def empirical_success_rate_private(self,
                                policy : np.ndarray,
                                num_trajectories : int = 1000,
                                max_steps_per_trajectory : int = 50,
                                epsilon=1,
        ...
        for t_ind in range(num_trajectories):
            np.random.seed()
            temp_traj = self.run_trajectory_private(policy, max_steps=max_steps_per_trajectory)
            if (temp_traj[-1] in self.target_indexes):
                    success_count = success_count + 1
    ...
    def build_privacy_mechanism(self, epsilon, k):
        ...
        # empty policy
        self.mu = np.zeros([one_agent_gridworld.Ns_local,one_agent_gridworld.Ns_local,one_agent_gridworld.Ns_local])
            ...
            for s_o_t_1 in range(one_agent_gridworld.Ns_local):
                ...
                for s_o_t in range(one_agent_gridworld.Ns_local):
                    if s == s_o_t and s in possible_transitions:
                        self.mu[s,s_o_t_1,s_o_t] = tau
                    elif s != s_o_t and s_o_t in possible_transitions:
                        self.mu[s,s_o_t_1,s_o_t] = (1-tau*int(s in possible_transitions))/(N_s_o_t_1-int(s in possible_transitions))
        return self.mu

    def run_trajectory_private(self, policy : np.ndarray, max_steps : int = 50, epsilon=1, k=3):
        
        #Construct a privacy mechanism for each agent

        self.build_privacy_mechanism(epsilon=epsilon,k=k) # everyone has the same epsilon for now
        ...
        last_private_state = {}
        for agent_id in range(self.N_agents):
            last_private_state[agent_id] = self.local_index_from_pos[s_tuple[2*agent_id:(2*agent_id+2)]]
        ...
        while ((s not in self.target_indexes) 
            ...
            for agent_id in range(self.N_agents):
                ...
                true_state = self.local_index_from_pos[s_tuple[2*agent_id:(2*agent_id+2)]]
                
                # Generate private state
                private_states[agent_id] = np.random.choice(np.arange(self.Ns_local), p=self.mu[true_state, last_private_state[agent_id], :])
            ...
            for agent_id in range(self.N_agents):
                ...
                for i in range(self.N_agents):
                    if i==agent_id:
                        ...
                        agent_i_s_hat=(agent_i_s_hat+self.local_pos_from_index[private_states[i]]) # use everyone elses private data
                # Get action disctribution based on the private information
                agent_i_s_hat_idx = self.index_from_pos[agent_i_s_hat]
                ...
                my_action = self.action_tuple_from_index[act][agent_id]
                agents_actions[agent_id]=my_action
               
            # Now that we have each agents action, make it a tuple joint action
            a_tuple = tuple(agents_actions[agent_id] for agent_id in range(self.N_agents))
            a_joint_id = self.action_index_from_tuple[a_tuple]
            
            s = np.random.choice(np.arange(self.Ns_joint), p=self.T[s,a_joint_id,:])
            s_tuple = self.pos_from_index[s]
